...
export function joinActivations(
  ...
  return {
    ...
    features: geojson.features.map((feature) => {
      // The top-level GeoJSON `id`, NOT `properties.key` -- there is no
      // such property. Verified against the real features.geojson: 2162
      // unique ids for 2162 features, and `engine/activation.py:209` builds
      // `FeatureMetrics(key=f["id"])`, so the payload's feature keys ARE
      // these ids. Reading `properties.key` yields undefined for every
      // feature, producing zero activations and a wholly muted map with no
      // error anywhere.
...
- [ ] **Step 5: Confirm the join key against the real `features.geojson`**

The join reads the top-level `feature.id`. Confirm that matches reality --
`properties` carries no `key` field, so a version of this join reading
`properties.key` would silently produce zero activations:
