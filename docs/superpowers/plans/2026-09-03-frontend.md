...
def depth_tint_png(src: Path, png: Path) -> dict:
    ...
    # np.interp needs ascending x, and depths are negative and descending.
    xs = -stops[::-1]
    rgba = np.zeros((flat.size, 4), dtype="uint8")
    for band in range(3):
        ys = cols[::-1, band]
        rgba[valid, band] = np.interp(-flat[valid], xs, ys).astype("uint8")
    rgba[valid, 3] = 255
...
```python
    ...
    def _regime_mix(slug: str, day):
        ...
        day_conditions = dayloader.load_day(fishery, day, "best", default_cache())
    ...
    def get_flow_vectors(slug: str, raw_day: str, hour: int, model: str = "best"):
        ...
        fishery, day_conditions, mix = _regime_mix(slug, day)
        events, ok = payload_mod._flow_events(fishery, day, default_cache())
        if not ok:
    ...
    def get_salinity_field(slug: str, raw_day: str, hour: int, model: str = "best"):
        ...
        fishery, day_conditions, _ = _regime_mix(slug, day)
        try:
...
Add the supporting imports `from tidescout.engine.phase import library_phase`, `from tidescout.engine import structure`, `from tidescout.pipeline import flowlib`, `from tidescout.sources import dayloader`, `from tidescout.sources.cache import default_cache` to `app.py`.
...
**Interfaces:**
- Produces:
  - `types.ts`: `DayPayload`, `SpeciesBlock`, `HourScore`, `SubScore`, `FeatureBlock`, `FeatureHour`, `Conditions`, `SubScope`, `FisherySummary`, `DayStatus`
